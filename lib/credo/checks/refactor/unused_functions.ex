defmodule Checkbook.Check.Refactor.UnusedPublicFunctions do
  @moduledoc """
  Finds unused public functions in the current project.
  """
  use Credo.Check,
  ...
  defp should_run?(%SourceFile{filename: filename}) do
    app_lib_path = Path.join(File.cwd!(), "lib")
    String.starts_with?(filename, app_lib_path)
  end
  ...
  defp gather_all_function_calls do
    app_lib_path = Path.join(File.cwd!(), "lib")
    Path.wildcard(Path.join(app_lib_path, "**/*.ex"))
